...
if ! command -v nix >/dev/null 2>&1; then
  echo "Error: nix package manager is not installed. Please install it first."
...
if ! command -v direnv >/dev/null 2>&1; then
  ...
  echo "You can install it via nix: nix-env -i direnv"
  echo "Or visit https://direnv.net/ for other installation methods."
  exit 1
fi
direnv allow
with_tests=false
